...
/** Shape used to build seed surveys with dates relative to `now`. */
interface SeedSpec {
  id: string;
  title: string;
  description: string;
...
function buildSurvey(spec: SeedSpec, now: Date): Survey {
  return {
    id: spec.id,
    title: spec.title,
    ...
    status: 'published',
    endsAt: atDaysFromNow(spec.endsOffset, now),
    createdAt: atDaysFromNow(spec.createdOffset, now),
...
export function createSeedSurveys(now: Date = new Date()): Survey[] {
  return [...ACTIVE_SPECS, ...PAST_SPECS].map((spec) => buildSurvey(spec, now));
}
